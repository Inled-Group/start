    ...
    Object.keys(dropdownData).forEach(dropdownId => {
      ...
      if (dropdown) {
        dropdownData[dropdownId].forEach(item => {
          ...
          button.textContent = item.text; // Changed from item.name to item.text to match your data structure
    ...
    window.buscadorActual = savedEngine; // Make it global
  ...
  window.buscadorActual = 'google'; // Make it global
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Enter key in search input field
  ...
  window.guardarFavorito = function() {
    ...
    if (!url.startsWith('http://') && !url.startsWith('https://')) {
      finalUrl = 'https://' + url;
      document.getElementById('url-favorito').value = finalUrl;
    }
